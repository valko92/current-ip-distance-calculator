...
Execute the command ``yarn`` or ``npm install`` to install dependencies.

Then run ``yarn start`` or ``npm start`` to run locally.

Or do ``yarn build`` or ``npm run build`` to bundle up and build a production-ready, minfied project folder. 
